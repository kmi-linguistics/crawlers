...
def getCommentData(datac, utube, chid, vid):
    ...
    for data in datac['items']:
        ...
        csv_data.append(['Comment'+str(ccount), final_text])
        ...
        if replies > 0 and 'replies' in data:
            ...
            for comment in replies_a:
                crcount += 1
                print ('Video:', video_count, '\tComment:', ccount, '\tCrawling reply', crcount, 'on the comment')
                rep = comment['snippet']
                ...
                # storing data for CSV. More information may be added later, based on requirement
                csv_data.append(['Comment ' + str(ccount) +
                                 '.' + str(crcount), final_text])
...
def getVideoData(datad):
    ...
    for data in datad['items']:
        ...
        if nlink not in prev_videos:
            csv_data.clear()
            meta.clear()

            # Getting Video Data
            ntitle = data['snippet']['title']
            urlv = "https://www.googleapis.com/youtube/v3/videos?part=snippet%2CcontentDetails%2Cstatistics&id="+nlink+"&key="+key
            ...
            for vdata in datav['items']:
                ...
                print ('Total comments on video', nlink, ':', cmntc)
                ...
                if int(cmntc) > 0:
                    ...
                    vLink = "https://www.youtube.com/watch?v="+nlink
                    ...
                    meta_video = []
                    meta_video.append(nlink)  # video ID
                    ...
                    urlc = "https://www.googleapis.com/youtube/v3/commentThreads?part=snippet,replies&videoId="+nlink+"&key="+key
                    with urllib.request.urlopen(urlc) as url:
                        datac = json.loads(url.read())

                    # get comments from first page
                    print ('Video:\t', video_count, '\tCrawling comment', ccount, 'on page 0 of the video')
                    utube = getCommentData(datac, utube, channel_id, nlink)
                    ...
                    print ('Total', totalPages, 'pages of comments in video', nlink)
                        ...
                        for i in range(totalPages):
                            if 'nextPageToken' in datac:
                                ...
                                urlc = "https://www.googleapis.com/youtube/v3/commentThreads?part=snippet,replies&videoId=" + \
                                    nlink+"&key="+key+"&pageToken="+pToken
                                #print (urlc)
                                try:
                                    with urllib.request.urlopen(urlc) as url:
                                        datac = json.loads(url.read())
                                    ...
                                    print ('Video:\t', video_count, '\tCrawling comment', ccount, 'on page', i, 'of the video')
                                    utube = getCommentData(
                                        datac, utube, channel_id, nlink)
                                except Exception as e:
                                    print (e)
                                    print (urlc)
                                    print ('Page', i, 'out of total', totalPages, 'pages of comments')
                                    print ('Expected total comments', totalResults)
                                    print ('Expected complete', totalPages*20)
                                    print (pToken)
...
# Function for retrieving the list of channels and users from where data is to be collected
def getList():
    global ytids
...
def getPreviousVideos():
    ...
    if os.path.exists('youTubeLinks.tsv'):
        with open('youTubeLinks.tsv') as f:
            ...
            for entry in reader:
                vid = entry[0]
                if vid != 'Video_ID' and vid not in prev_videos:
...
# Get list of channel / users to retrieve data from
getList()
...
print (len(ytids), 'new channels / users to be collected')
...
# Iterating through each channel / user
