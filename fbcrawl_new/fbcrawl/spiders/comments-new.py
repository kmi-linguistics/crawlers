...
class CommentsSpider(FacebookSpider):
    ...
    def parse_page(self, response):
        '''
        '''
        print ('Into parse_page', response)
        print (type (response), type (response.url),response.url)

        working_dir = os.getcwd()
        #working_dir = root_dir[:root_dir.rfind('/')] + '/posts'
        
        print ('Working Dir', working_dir)
        if self.type == 'post':
            for cur_dir_path, subdirs, files in os.walk(working_dir):
                for curnt_file in files:
                    ...
                    if file_path.endswith ('.csv'):
                       with open (file_path, 'r') as f:
                           ...
                           for row in reader:
                               if row[12] != 'post_id':
                                  resp_url = "https://mbasic.facebook.com" + row[13]
                                  print ('Current response URL', resp_url)
                                  yield scrapy.Request(url=resp_url, 
